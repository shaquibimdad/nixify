ln -s ~/mynixconfigs/.config/bspwm/ ~/.config/bspwm
ln -s ~/mynixconfigs/.config/polybar/ ~/.config/polybar
ln -s ~/mynixconfigs/.config/sxhkd/ ~/.config/sxhkd
ln -s ~/mynixconfigs/.config/plank/ ~/.config/plank
ln -s ~/mynixconfigs/.config/gtk-2.0/ ~/.config/gtk-2.0
ln -s ~/mynixconfigs/.config/gtk-3.0/ ~/.config/gtk-3.0
ln -s ~/mynixconfigs/.config/rofi/ ~/.config/rofi
ln -s ~/mynixconfigs/.config/alacritty ~/.config/alacritty
ln -s ~/mynixconfigs/.config/awesome ~/.config/awesome
ln -s ~/mynixconfigs/.config/picom ~/.config/picom
echo "Done"
